...
| Surfaces | Anything carrying body text over the Extreme theme's animated canvas must be opaque. `--surface` is 5% white there; answer rows used it and the canvas was legible through the text. Question cards use `--field`, answer rows use `--opt`. |
